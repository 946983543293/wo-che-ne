 ...
 * @since
 *
 * 【本仓库 vendoring 改造】原版使用 {@code io.flutter.view.FlutterMain}（v1 嵌入时代 API，
 * 已被 Flutter 引擎移除）。asset 查找键改由 {@link FlutterInjector#flutterLoader()} 提供，
 * 语义与旧 API 完全一致（flutter_assets/... 前缀 + package 前缀）。
 */
public class ConvertUtil {
    ...
    /** 【vendoring 改造】替代已移除的 {@code FlutterMain.getLookupKeyForAsset(String)}。 */
    private static String lookupKeyForAsset(String asset) {
        return FlutterInjector.instance().flutterLoader().getLookupKeyForAsset(asset);
    }

    /** 【vendoring 改造】替代已移除的 {@code FlutterMain.getLookupKeyForAsset(String, String)}。 */
    private static String lookupKeyForAsset(String asset, String packageName) {
            ...
            case "fromAsset":
                if (data.size() == 2) {
                    return BitmapDescriptorFactory.fromAsset(
                            lookupKeyForAsset(toString(data.get(1))));
                } else {
                    return BitmapDescriptorFactory.fromAsset(
                            lookupKeyForAsset(toString(data.get(1)), toString(data.get(2))));
                }
            case "fromAssetImage":
                if (data.size() == 3) {
                    return BitmapDescriptorFactory.fromAsset(
