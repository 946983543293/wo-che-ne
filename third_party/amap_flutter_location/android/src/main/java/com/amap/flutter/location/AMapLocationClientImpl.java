...
public class AMapLocationClientImpl implements AMapLocationListener {
    ...
    public AMapLocationClientImpl(Context context, String pluginKey) {
        mContext = context;
    ...
    public void onLocationChanged(AMapLocation location) {
        // 【vendoring 修复】EventChannel 的 EventSink 要等 Dart 侧首次 listen 之后才就绪，
        // 不能在构造 AMapLocationClientImpl 时缓存（那时必然是 null，会导致首个定位请求
        // 永久收不到回调）。改为每次回调时实时读取插件当前的 sink。
        EventChannel.EventSink sink = AMapFlutterLocationPlugin.mEventSink;
        if (null == sink) {
            return;
        }
